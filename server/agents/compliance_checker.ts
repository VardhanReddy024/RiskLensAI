...
export async function runComplianceAgent(transaction: Transaction, riskScore: number): Promise<ComplianceAgentResult> {
  ...
  const isCryptoOrBullion = /crypto|bullion|gold|wire/i.test(transaction.merchant) || transaction.merchantCategory === 'Crypto Exchange';
  ...
  if (!transaction.paymentMethod.is3DSecure && transaction.amount > 250) {
  ...
  const compliance: ComplianceCheckResult = {
    ...
    psd3ScaRequired: !transaction.paymentMethod.is3DSecure,
    triggeredRules,
    notes: sarRequired 
