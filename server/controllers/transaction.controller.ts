...
export class TransactionController {
  ...
  public static async getTransactionById(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
      const found = await transactionService.getTransactionById(id, req.tenantId);
  ...
  public static async ingestBatch(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { transactions, replaceExisting } = req.body;
      const result = await transactionService.ingestBatch(transactions, req.tenantId, Boolean(replaceExisting));
      res.status(200).json(result);
    } catch (err) {
      next(err);
    }
