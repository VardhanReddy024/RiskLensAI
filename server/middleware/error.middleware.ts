...
export function errorHandlerMiddleware(err: any, req: Request, res: Response, next: NextFunction): void {
  ...
  // 3. Return standardized JSON response with production-safe messages
  const isProduction = process.env.NODE_ENV === 'production';
  const clientMessage = (appError.statusCode >= 500 && isProduction)
    ? 'An unexpected internal server error occurred.'
    : appError.message;

  const clientDetails = (isProduction && appError.statusCode >= 500)
    ? undefined
    : appError.details;
