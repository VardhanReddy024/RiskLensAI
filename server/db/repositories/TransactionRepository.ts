...
export class TransactionRepository implements ITransactionRepository {
  ...
  public async clearAll(tenantId?: string): Promise<void> {
    await this.adapter.clearTransactions(tenantId);
  }
  ...
  public async getAnalyticsMetrics(recentLogsLimit = 10, tenantId?: string): Promise<AnalyticsMetricsResult> {
    const all = await this.adapter.getAllTransactions(tenantId ? { tenantId } : undefined);
