...
export class InMemoryAdapter implements IDataStoreAdapter {
  ...
  public async getAllTransactions(filter?: TransactionFilter): Promise<Transaction[]> {
    ...
    if (filter) {
      if (filter.tenantId) {
        // Strict tenant isolation: Tenant A never sees Tenant B. Default/platform seed records are only visible to internal RiskLens operators.
        const isInternalTenant = filter.tenantId === 'default_tenant' || filter.tenantId === 'tenant_risklens_ai' || filter.tenantId === 'tenant_risklens';
        list = list.filter(t => {
          if (t.tenantId === filter.tenantId) return true;
          if (!t.tenantId || t.tenantId === 'default_tenant') {
            return isInternalTenant;
          }
          return false;
        });
      }
      if (filter.provider) {
        list = list.filter(t => t.provider === filter.provider);
      }
  ...
  public async clearTransactions(tenantId?: string): Promise<void> {
    ...
    for (const [id, transaction] of this.transactions) {
      if (transaction.tenantId === tenantId) this.transactions.delete(id);
    }
  }
  ...
  public async claimWebhookEvent(event: WebhookEventRecord): Promise<boolean> {
    ...
    this.webhookEvents.set(key, JSON.parse(JSON.stringify(event)));
    return true;
  }
  ...
  public async saveWebhookEvent(event: WebhookEventRecord): Promise<WebhookEventRecord> {
    ...
    this.webhookEvents.set(key, cloned);
    return cloned;
  }
}
