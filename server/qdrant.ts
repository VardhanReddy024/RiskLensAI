...
export function searchSimilarFraudCases(
  ...
  // Extract key transaction feature weights safely
  const amount = transaction.amount || 0;
  const isHighAmount = amount > 2500;
  const isCryptoOrBullion = /crypto|coin|bullion|gold|wire/i.test(transaction.merchant || '') || transaction.merchantCategory === 'Crypto Exchange' || transaction.merchantCategory === 'Crypto';
  const isProxyOrTor = !!(transaction.ipAddress?.isProxy || transaction.ipAddress?.isTor || transaction.ipAddress?.isVpn);
  const isImpossibleTravel = (transaction.location?.distanceFromHomeKm ?? 0) > 1000;
  const isEmulator = transaction.device?.type === 'Bot/Emulator' || (transaction.device?.fingerprintScore ?? 100) < 35;
  const isRapidVelocity = (transaction.tags || []).some(t => /velocity|rapid/i.test(t));
  const isWire = transaction.paymentMethod?.type === 'Wire Transfer';
  ...
  const scoredCases = HISTORICAL_FRAUD_CASES.map(historicalCase => {
    ...
    if (historicalCase.fraudType.includes('Mule') && isWire) {
      baseSimilarity += 0.20;
    }
    if (historicalCase.fraudType.includes('Card Testing') && amount < 5) {
      baseSimilarity += 0.26;
    }
    if (isProxyOrTor) {
      baseSimilarity += 0.08;
    ...
    const charCodeSum = (transaction.id || 'TX-DEFAULT').split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
