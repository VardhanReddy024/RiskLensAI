...
export function buildGraphForTransaction(
  ...
  // 6. Location Node
  const locAny = transaction.location as any;
  const latVal = locAny.latitude ?? transaction.location.lat;
  const lonVal = locAny.longitude ?? transaction.location.lon;
  const locDetails = (latVal !== undefined && lonVal !== undefined)
    ? `Lat: ${Number(latVal).toFixed(4)}, Lon: ${Number(lonVal).toFixed(4)}`
  ...
  const pmAny = transaction.paymentMethod as any;
  const issuerName: string = pmAny.bankIssuer || transaction.paymentMethod.issuer || 'Direct Clearing Issuer';
  const last4: string = pmAny.cardLast4 || transaction.paymentMethod.last4 || '0000';
  const cardCountry: string = pmAny.cardCountry || transaction.location.country;

  const accountNode: GraphNode = {
    id: `ACC-${issuerName.replace(/[^a-zA-Z0-9]/g, '_')}-${last4}`,
    label: `${issuerName} •••• ${last4}`,
    ...
    aiSummary: `Settlement instrument issued by ${issuerName} in ${cardCountry}. 3DS: ${transaction.paymentMethod.is3DSecure ? 'Authenticated' : 'Unsecured'}.`,
    importance: 7,
  };
  addNode(accountNode);
