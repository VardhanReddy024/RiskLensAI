...
describe('RiskLens AI Persistent Database Layer Suite', () => {
  ...
  describe('7. PgAdapter & PostgreSQL Integration Layer', () => {
    it('initializes PgAdapter with memory fallback when DATABASE_URL is not set in non-strict mode', async () => {
    ...
    it('refuses silent fallback and throws error when STRICT_DATABASE=true and DATABASE_URL is missing', async () => {
      const { PgAdapter } = await import('../../server/db/adapters/PgAdapter');
      const originalStrict = process.env.STRICT_DATABASE;
      ...
      const pgAdapter = new PgAdapter();
      await expect(pgAdapter.initialize()).rejects.toThrow(/Strict Mode/);
