...
import { evaluateFraudModel, evaluateTransactionWithML, FRAUD_MODEL_EVALUATION } from '../lib/ml_engine';
import { FRAUD_MODEL_THRESHOLDS, FRAUD_TEST_ROWS, FRAUD_TRAINING_ROWS } from '../data/fraud_dataset';
...
describe('ML Inference & SHAP Explainability Engine', () => {
  ...
  it('correctly evaluates low-risk benign transactions', () => {
    ...
    expect(result.modelDetails.algorithm).toContain('logistic regression');
    expect(result.modelDetails.aucRoc).toBeGreaterThan(0.5);
    expect(result.modelDetails.evaluation).toEqual(FRAUD_MODEL_EVALUATION);
    expect(result.shapFactors.length).toBeGreaterThanOrEqual(4);

    // Contributions remain model-derived even when the aggregate probability is low.
    const amountFactor = result.shapFactors.find(f => f.feature === 'amount_z_score');
    expect(amountFactor).toBeDefined();
    expect(Number.isFinite(amountFactor?.impactScore)).toBe(true);
  });
  ...
  it('triggers critical risk and high SHAP attribution for high-velocity anomalous transaction', () => {
    ...
    expect(amountShap?.impactScore).toBeGreaterThan(0);
    expect(amountShap?.isSuspicious).toBe(true);

    const geoShap = result.shapFactors.find(f => f.feature === 'distance_score');
    expect(geoShap?.impactScore).toBeGreaterThan(0);
    expect(geoShap?.isSuspicious).toBe(true);

    const deviceShap = result.shapFactors.find(f => f.feature === 'device_risk');
    expect(deviceShap?.impactScore).toBeGreaterThan(0);
    expect(deviceShap?.isSuspicious).toBe(true);

    const ipShap = result.shapFactors.find(f => f.feature === 'network_risk');
    expect(ipShap?.impactScore).toBeGreaterThan(0);
    expect(ipShap?.isSuspicious).toBe(true);

    const merchantShap = result.shapFactors.find(f => f.feature === 'merchant_risk');
    expect(merchantShap?.impactScore).toBeGreaterThan(0);
    expect(merchantShap?.isSuspicious).toBe(true);

    const authShap = result.shapFactors.find(f => f.feature === 'authentication_risk');
    expect(authShap?.impactScore).toBeGreaterThan(0);
  ...
  it('handles intermediate tier transitions accurately (Elevated amount, out of state, commercial VPN)', () => {
    ...
    const amountFactor = result.shapFactors.find(f => f.feature === 'amount_z_score');
    expect(amountFactor?.impactScore).toBeGreaterThan(0);

    const geoFactor = result.shapFactors.find(f => f.feature === 'distance_score');
    expect(geoFactor?.impactScore).toBeGreaterThan(0);

    const deviceFactor = result.shapFactors.find(f => f.feature === 'device_risk');
    expect(deviceFactor?.impactScore).toBeGreaterThan(0);

    const ipFactor = result.shapFactors.find(f => f.feature === 'network_risk');
    expect(ipFactor?.impactScore).toBeGreaterThan(0);
  });
  ...
  it('evaluates only the deterministic held-out set and reports threshold costs', () => {
    const evaluation = evaluateFraudModel();

    expect(evaluation.trainRows).toBe(FRAUD_TRAINING_ROWS.length);
    expect(evaluation.testRows).toBe(FRAUD_TEST_ROWS.length);
    expect(evaluation.trainRows + evaluation.testRows).toBeGreaterThan(0);
    expect(evaluation.threshold).toBe(FRAUD_MODEL_THRESHOLDS.reviewBelow);
    ...
    expect(evaluation.falsePositiveCostUsd).toBe(
      evaluation.falsePositiveCount * FRAUD_MODEL_THRESHOLDS.falsePositiveCostUsd
    );
  });
});
