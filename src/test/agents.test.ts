...
describe('Multi-Agent AI Autonomous Consensus Architecture', () => {
  ...
  it('Deep Investigate handles dynamic live streamed transactions with transactionOverride', async () => {
    const { db } = await import('../../server/db');
    await db.initialize();
    const { investigationService } = await import('../../server/services/investigation.service');
    const liveTxn: Transaction = {
      ...sampleSuspiciousTxn,
      id: 'TXN-LIVE-040187',
    };

    const result = await investigationService.investigate('TXN-LIVE-040187', liveTxn, 'tenant_test');
    expect(result.success).toBe(true);
    expect(result.dossier).toBeDefined();
    expect(result.dossier.transaction.id).toBe('TXN-LIVE-040187');
    expect(result.dossier.id).toContain('TXN-LIVE-040187');
  });
});
