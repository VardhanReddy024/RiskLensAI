...
function mapFirebaseErrorToMessage(errorCode: string): string {
  switch (errorCode) {
    ...
    case 'auth/configuration-not-found':
      return 'Firebase Authentication is not configured. Please provide VITE_FIREBASE_API_KEY in your .env or .env.local file.';
    case 'auth/invalid-api-key':
    case 'auth/api-key-not-valid':
      return 'Firebase API key is invalid. Please verify VITE_FIREBASE_API_KEY in your .env or .env.local file.';
...
describe('Authentication Error Handling', () => {
  describe('Firebase error code → user message mapping', () => {
    ...
    it('maps auth/configuration-not-found to environment setup instructions', () => {
      const msg = mapFirebaseErrorToMessage('auth/configuration-not-found');
      expect(msg).toContain('VITE_FIREBASE_API_KEY');
      expect(msg).toContain('configured');
    });

    it('maps auth/invalid-api-key to key verification error', () => {
      const msg = mapFirebaseErrorToMessage('auth/invalid-api-key');
  ...
  describe('Safe unconfigured Firebase listener', () => {
    it('notifies observer with null when auth instance is null without crashing', async () => {
      const { onAuthStateChanged } = await import('../lib/firebase');
      let receivedUser: any = undefined;
      const unsubscribe = onAuthStateChanged(null, (user: any) => {
        receivedUser = user;
      });
