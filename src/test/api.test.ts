...
describe('RiskLens AI Express API Integration Suite (Supertest)', () => {
  ...
  it('GET /api/health returns 200 OK and valid service health status (Public)', async () => {
  ...
  it('GET /api/transactions rejects unauthenticated requests with 401 Unauthorized', async () => {
    const res = await request(app).get('/api/transactions');
    expect(res.status).toBe(401);
    expect(res.body.code).toBe('UNAUTHORIZED');
  });

  it('GET /api/transactions returns seeded transactions with valid auth token', async () => {
    // 1. Unfiltered with auth
    ...
    const searchRes = await request(app)
      .get('/api/transactions?search=Crypto')
      .set(authHeader);
  ...
  it('GET /api/transactions/:id retrieves single transaction or returns 404', async () => {
    ...
    const detailRes = await request(app)
      .get(`/api/transactions/${firstTxn.id}`)
      .set(authHeader);
    ...
    const notFoundRes = await request(app)
      .get('/api/transactions/NON-EXISTENT-ID-999')
      .set(authHeader);
  ...
  it('POST /api/investigate/:id executes 8-agent pipeline and returns cached results on repeat', async () => {
    ...
    const invRes = await request(app)
      .post(`/api/investigate/${targetTxn.id}`)
      .set(authHeader);
