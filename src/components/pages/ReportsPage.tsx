        ...
        <div className="enterprise-card p-7 sm:p-8 space-y-7">
          <div className="border-b border-slate-200/80 pb-5">
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Prepared by: <strong className="text-slate-700 font-semibold">{currentUser?.displayName || 'Reddy Vardhan Kumar'}</strong> ({currentUser?.role || 'Senior Fraud Analyst'}) • Date: {new Date().toLocaleDateString()}
            </p>
          </div>
      ...
      {selectedReportType === 'audit' && (
        <div className="enterprise-card-static overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 text-xs">
            {[
              { time: '10 mins ago', user: currentUser?.displayName || 'Reddy Vardhan Kumar', role: currentUser?.role || 'Senior Fraud Analyst', action: 'REJECT_TRANSACTION', target: 'TXN-98425-FRAUD', hash: '0x8f2a...11b4' },
              { time: '35 mins ago', user: currentUser?.displayName || 'Reddy Vardhan Kumar', role: currentUser?.role || 'Senior Fraud Analyst', action: 'GENERATE_SAR', target: 'TXN-98425-FRAUD', hash: '0x3c1d...99e2' },
              { time: '1 hr ago', user: currentUser?.displayName || 'Reddy Vardhan Kumar', role: currentUser?.role || 'Senior Fraud Analyst', action: 'HOLD_TRANSACTION', target: 'TXN-98426-FLAG', hash: '0x7e44...aa81' },
