...
export function DashboardPage({
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Active User Uploaded Dataset Indicator */}
      {uploadSummary && (
        <div className="p-4 sm:p-5 rounded-2xl bg-blue-50/90 border border-blue-200/90 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs sm:text-sm font-bold text-blue-950">
                Active Analysis Source: <span className="font-mono text-blue-800">{uploadSummary.fileName || 'Uploaded Dataset'}</span>
              </div>
              <div className="text-xs text-blue-700 font-medium">
                {uploadSummary.validRows} user transactions loaded into active queue and risk matrix • Demo dataset replaced
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={onNavigateToUpload}
              className="btn-premium-secondary px-3 py-1.5 text-xs text-blue-900 border-blue-200"
            >
              Upload New File
            </button>
            <button
              onClick={clearUploadedDataset}
              className="px-3 py-1.5 rounded-xl text-xs font-bold text-slate-600 hover:text-slate-900 hover:bg-white/80 transition-all border border-slate-200"
            >
