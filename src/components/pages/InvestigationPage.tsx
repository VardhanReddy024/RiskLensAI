...
import { safeRenderText } from '../../lib/error_normalizer';
import { 
  ShieldAlert, 
  Sparkles, 
  Database, 
...
export function InvestigationPage({ initialTransaction, onNavigateToDashboard }: InvestigationPageProps) {
  ...
  // If no active dossier, load from initialTransaction, URL parameter, or first available transaction
  useEffect(() => {
    if (activeDossier || isInvestigating) return;
    ...
    if (typeof window !== 'undefined') {
      ...
      const urlTxnId = searchParams.get('transactionId') || searchParams.get('id');
      if (urlTxnId) {
        const found = transactions.find(t => t.id === urlTxnId);
        if (found) {
          startInvestigation(found);
          return;
        }
      }
    }

    const target = transactions.find(t => t.riskTier === 'CRITICAL' || t.riskTier === 'HIGH') || transactions[0];
    if (target) {
      startInvestigation(target);
    }
  }, [initialTransaction, activeDossier, isInvestigating, transactions, startInvestigation]);

  // When initialTransaction explicitly changes
  useEffect(() => {
    if (initialTransaction && activeDossier?.transaction.id !== initialTransaction.id && !isInvestigating) {
      startInvestigation(initialTransaction);
    }
  }, [initialTransaction, activeDossier?.transaction.id, isInvestigating, startInvestigation]);
          ...
          <div className="enterprise-card p-0 flex flex-col h-[540px] overflow-hidden">
              ...
              {activeDossier.chatHistory.map((msg) => (
                ...
                >
                  ...
                  <div className={`p-3 rounded-2xl max-w-[85%] leading-relaxed ${
                    ...
                    <p className="text-[11px] whitespace-pre-wrap">{safeRenderText(msg.message)}</p>
            ...
            <div className="px-3 py-2 border-t border-slate-100 bg-white/90 flex items-center gap-1.5 overflow-x-auto shrink-0 text-[11px]">
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 shrink-0 transition-colors font-medium disabled:opacity-50"
              >
                Why flagged?
              </button>
              <button
              ...
              <button
                type="button"
                disabled={isSendingMessage}
                onClick={() => sendCopilotMessage("Explain the device risk.")}
                className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 shrink-0 transition-colors font-medium disabled:opacity-50"
              >
                Device Risk
              </button>
              <button
                type="button"
                disabled={isSendingMessage}
                onClick={() => sendCopilotMessage("What is the recommended action?")}
                className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 shrink-0 transition-colors font-medium disabled:opacity-50"
              >
                Recommended Action
              </button>
