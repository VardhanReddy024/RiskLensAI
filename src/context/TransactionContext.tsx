...
import { DatasetParseResult } from '../lib/dataset_parser';
import { useAuth } from './AuthContext';

export interface UploadSummary extends Partial<DatasetParseResult> {
  fileName?: string;
  uploadedAt?: string;
}
...
interface TransactionContextType {
  ...
  ingestBatch: (txns: Transaction[], options?: { replaceExisting?: boolean; summary?: UploadSummary }) => Promise<void>;
...
export function TransactionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Only fetch protected transaction data when authentication state has resolved and user is authenticated
    if (!isAuthLoading && isAuthenticated) {
      refreshTransactions();
    }
  }, [isAuthLoading, isAuthenticated, refreshTransactions]);
  ...
  const ingestBatch = useCallback(async (
    newTxns: Transaction[],
    options?: { replaceExisting?: boolean; summary?: UploadSummary }
  ) => {
    setIsLoading(true);
    const replaceExisting = options?.replaceExisting ?? false;

    if (options?.summary) {
      setUploadSummary(options.summary);
    }
    ...
    try {
      ...
      throw err instanceof Error ? err : new Error('Unable to save transaction');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const clearUploadedDataset = useCallback(async () => {
    try {
      ...
      throw e instanceof Error ? e : new Error('Unable to save transaction');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const activeTenantId = currentUser?.email
        ? (currentUser.email.includes('@') ? `tenant_${currentUser.email.split('@')[1].toLowerCase().replace(/[^a-z0-9]/g, '_')}` : 'default_tenant')
        : 'default_tenant';
  ...
  }, [isStreaming, streamSpeed, currentUser]);
